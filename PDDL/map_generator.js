...
module.exports = {
  generate: function (height, width, blockedCells, numberOfMonstersPerType) {
    ...
    for (var i = 0; i < blockedCells; i++) {
      var newBlockIndex;
      do {
         newBlockIndex = Math.floor(seedrandom() * numberOfCells);
    ...
    for (var i = 0; i < numberOfMonstersPerType.length; i++) {
      ...
      for (var j = 0; j < numberOfMonstersPerType[i]; j++) {
        problem += `m${i}${j} `;
        const newMonsterIndex = Math.floor(seedrandom() * possibleLocations.length);
      ...
      const newWeaponIndex = Math.floor(seedrandom() * possibleLocations.length);
